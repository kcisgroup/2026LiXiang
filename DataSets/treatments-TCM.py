...
def get_treatment_advice(symptom, treatment_type):
    prompt_templates = [ f"请为{symptom}提供中医的推荐治疗方案，若该疾病没有中医治疗方案则不生成。", f"如果一个患者得了{symptom}，中医的治疗方案是什么？若该疾病没有中医治疗方案则不生成", f"{symptom}的常见中医治疗方法有哪些？若该疾病没有中医治疗方案则不生成。", ]
...
def generate_treatment_dataset():
    ...
    for symptom in symptoms:
        # 获取西医治疗方法
        treatment_western = get_treatment_advice(symptom, '中医')
        print(f"中医治疗方法：{treatment_western}")

        # 将结果添加到列表中
        treatments.append([symptom, treatment_western])

    # 保存结果为CSV文件
    with open('../Datasets/train-TM.csv', 'a', newline='', encoding='utf-8') as file:
